...
async def find_circular_barters(request: CircularBarterRequest):
    """
    Find circular barter opportunities using graph theory.

    Algorithm:
    1. Build a directed graph where edge A→B exists if A has a skill B wants
    2. Find all simple cycles of length 3 to max_chain_length using DFS
    3. Score each cycle by skill match quality
    4. Return top cycles sorted by confidence score

    Path format: [(user_id, skill_this_user_gives), ...]
    A cycle [(A, X), (B, Y), (C, Z)] means:
      A gives X to B, B gives Y to C, C gives Z to A
    """
    users = request.users
    if len(users) < 3:
        return CircularBarterResponse(
            chains=[], total_users_analyzed=len(users), source="graph_algorithm"
        )

    # Build user lookup
    user_map = {u.user_id: u for u in users}

    # Build directed graph: edges[A] = [(B, skill_A_gives), ...]
    # An edge from A to B means: A has something B wants
    edges: dict[str, list[tuple[str, str]]] = {u.user_id: [] for u in users}

    for giver in users:
        for receiver in users:
            if giver.user_id == receiver.user_id:
                continue
            giver_skills = set(s.lower() for s in giver.has_skills)
            receiver_wants = set(s.lower() for s in receiver.wants_skills)
            matching_skills = giver_skills & receiver_wants

            if matching_skills:
                skill = matching_skills.pop()
                # Find original casing from giver's skill list
                give_skill = next((s for s in giver.has_skills if s.lower() == skill), skill)
                edges[giver.user_id].append((receiver.user_id, give_skill))

    # DFS-based cycle detection
    # Path stores: [(user_id, skill_this_user_gives), ...]
    detected_cycles: list[list[tuple[str, str]]] = []
    ...
    def find_cycles_from(
        ...
        """DFS to find cycles. 'current' is where we are; 'path' has confirmed entries."""
        if len(detected_cycles) >= request.max_results * 3:
            return

        for next_user, give_skill in edges.get(current, []):
            if next_user == start and len(path) >= 2:
                # Found a cycle! Add current's contribution and close the loop
                complete = path + [(current, give_skill)]
            ...
            find_cycles_from(start, path + [(current, give_skill)], next_user, visited)
            visited.remove(next_user)

    # Try starting from each user
    for user in users:
        visited = {user.user_id}
        # Follow each outgoing edge from the start user
        for next_user, give_skill in edges.get(user.user_id, []):
            visited.add(next_user)
            # Path starts with what this user gives; then DFS continues from next_user
            find_cycles_from(
                start=user.user_id,
                path=[(user.user_id, give_skill)],
                current=next_user,
                visited=visited,
            )
            visited.remove(next_user)

    # Deduplicate cycles (same participants in different rotation order)
    ...
    # Build structured chains with confidence scores
    result_chains: list[DetectedChain] = []

    for cycle in unique_cycles[:request.max_results]:
        chain_links = []
        n = len(cycle)  # cycle is [(user_id, give_skill), ...] — no closing entry

        for i, (uid, give_skill) in enumerate(cycle):
            next_idx = (i + 1) % n
            prev_idx = (i - 1) % n
            next_uid = cycle[next_idx][0]
            prev_uid = cycle[prev_idx][0]

            # What this user receives = what the previous user gives
        ...
        # Confidence = average of how well each link's skills match
        confidence = 100.0  # Perfect if we got here (exact skill name match)
        # Reduce confidence for longer chains (harder to coordinate)
        confidence *= (1.0 - (n - 3) * 0.1)  # 3-person = 100%, 4 = 90%, 5 = 80%
    ...
    # Sort by confidence descending, then by shorter chains first
