...
export default function ExplorePage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="reveal font-display text-3xl">Explore Creatives</h1>
        <p className="text-stone-400 mt-1">Discover talented people in your area</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-stone-900 rounded-2xl border border-stone-800">
          <p className="text-stone-400">No creatives found. Try adjusting your filters.</p>
        </div>
      ) : (
          ...
          {users.map((u: any) => (
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                <div className="w-12 h-12 bg-brand-500/20 rounded-full flex items-center justify-center text-brand-400 text-lg font-bold">
                  {u.name?.charAt(0) || '?'}
                </div>
                <div>
              ...
              {u.bio && (
                <p className="text-sm text-stone-400 line-clamp-2 mb-3">{u.bio}</p>
              )}
              ...
              {u.skills && u.skills.length > 0 && (
                <div className="flex flex-wrap gap-1 mb-3">
                  {u.skills.slice(0, 4).map((us: any) => (
                    <span key={us.id} className="text-xs bg-brand-500/10 text-brand-300 px-2 py-0.5 rounded-full">
                      {us.skill.name}
                    </span>
                  ))}
                  {u.skills.length > 4 && (
                    <span className="text-xs text-stone-400">+{u.skills.length - 4} more</span>
                  )}
                </div>
              )}

              {/* Footer */}
              <div className="flex items-center justify-between text-xs text-stone-400 pt-3 border-t border-stone-800/50">
                <span>
